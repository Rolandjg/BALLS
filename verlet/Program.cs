...
class Program
{
    ...
    private static void LeftMouseLogic(HashSet<Verlet> verlets, int frameNumber)
    {
        if (Raylib.IsMouseButtonDown(MouseButton.Left))
        {
            if (frameNumber % 2 == 0)
            {
                ...
                Verlet verlet = new Verlet(new Vector2(mouseX, mouseY), new Vector2(0, 1f), 2, color);
                verlets.Add(verlet);
            }
        }
    }
    ...
    private static void RightMouseLogic(HashSet<Verlet> verlets, Dictionary<Vector2, Color> faucets, int frameNumber)
    {
        ...
        if (Raylib.IsMouseButtonPressed(MouseButton.Right))
        {
            ...
            faucets.Add(new Vector2(mouseX, mouseY), new Color(rng.Next(0, 255), rng.Next(0, 255), rng.Next(0, 255), 255));
        }
        ...
        if (frameNumber % 2 == 0)
        {
            ...
            {
                Verlet verlet = new Verlet(faucet, new Vector2(0, 1), 2, faucets[faucet]);
                verlets.Add(verlet);
            }
        }
    }
